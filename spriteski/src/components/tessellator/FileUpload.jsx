...
export function FileUpload({ image, onImage }) {
		...
		<>
			...
			/>
			{
				image ? (
					<Canvas
						canvas={ image }
						className={ `p-4 border border-gray-200 hover:border-gray-300 border-solid rounded shadow-lg cursor-pointer m-auto` }
						onClick={ handleButtonClick }
					/>
				) : (
					<div
						className="relative flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-md cursor-pointer border-neutral-300 bg-neutral-50 hover:bg-neutral-100 hover:border-neutral-400"
						onClick={ handleButtonClick }
					>

...
import { byGrid2 as byGrid } from "./../modules/tiles/tessellators/Grid.js";
...
export function Tessellator() {
	...
	const calculateRowsAndColumns = (type = 0) => {
		let threshold = 1;
		if(type === 0) {
			threshold = Math.ceil(Math.sqrt(tileset.tiles.length));
		} else if(type === 1) {
	...
	return (
		<div className="flex flex-col h-screen gap-4 m-8">
			...
			<div className="m-auto">
				<ImagePreview canvasRef={ canvasRef } hide={ !image } />
						...
						<div className="flex flex-col items-center w-full">
							<div className="flex flex-row gap-2">
								<InputText
									...
									onChange={ e => dispatchTileset({ size: ~~e.target.value }) }
								/>
							</div>
							...
							<div className="flex flex-row gap-2">
								...
								<button
									className="px-4 py-2 mt-4 text-gray-700 border border-solid rounded bg-neutral-50 hover:bg-neutral-100 border-neutral-300 hover:border-neutral-400"
									onClick={ e => setIsRowXCol(!isRowXCol) }
									style={ { fontFamily: "'Fredoka One', cursive" } }
								>
